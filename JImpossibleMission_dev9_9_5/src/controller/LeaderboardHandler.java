...
public class LeaderboardHandler {
	...
	public void refreshLeaderboards() {
		...
		topByScore = allProfiles.stream().sorted(Comparator.comparingLong(UserProfile::getTotalScore).reversed())
				.limit(10).collect(Collectors.toList());

		// 2. Classifica Playtime (Tempo di Gioco Totale)
		topByPlaytime = allProfiles.stream()
				.sorted(Comparator.comparingLong(UserProfile::getTotalPlaytimeMs).reversed()).limit(10)
				.collect(Collectors.toList());

		// 3. Classifica Punteggio Medio
		topByAvgScore = allProfiles.stream().sorted(Comparator.comparingDouble(UserProfile::getAverageScore).reversed())
				.limit(10).collect(Collectors.toList());
	}
	...
	public boolean isBackHover() {
		return backHover;
	}

	public List<UserProfile> getTopByScore() {
		return topByScore;
	}

	public List<UserProfile> getTopByPlaytime() {
		return topByPlaytime;
	}

	public List<UserProfile> getTopByAvgScore() {
		return topByAvgScore;
	}
}